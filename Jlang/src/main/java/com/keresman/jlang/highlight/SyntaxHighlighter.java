...
public class SyntaxHighlighter {
    ...
    public void highlight(String sourceCode) {
        ...
        for (Token token : tokens) {
            SimpleAttributeSet style = switch (token.type()) {
                case INTERFACE, STRUCT, VOID, VAR, IF, ELSE, RETURN, INT32 ->
                    keywordStyle();
                case IDENTIFIER ->
                    identifierStyle();
                case STRING_LITERAL ->
                ...
                case NUMBER_LITERAL ->
                    numberStyle();
                case EQUAL_EQUAL, NOT_EQUAL, ARROW ->
                    operatorStyle();
                case L_BRACE, R_BRACE, L_PAREN, R_PAREN, SEMICOLON, COMMA ->
                    punctuationStyle();
                default ->
                    defaultStyle();
            };
