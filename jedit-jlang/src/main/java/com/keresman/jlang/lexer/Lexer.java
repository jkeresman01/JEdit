...
public class Lexer {
  ...
  private void scanToken() {
    ...
    switch (c) {
      case '{' -> addToken(TokenType.L_BRACE);
      case '}' -> addToken(TokenType.R_BRACE);
      case '(' -> addToken(TokenType.L_PAREN);
      case ')' -> addToken(TokenType.R_PAREN);
      case ';' -> addToken(TokenType.SEMICOLON);
      case ',' -> addToken(TokenType.COMMA);
      case '.' -> addToken(TokenType.DOT);
      case '*' -> addToken(TokenType.STAR);
      case '=' -> addToken(match('=') ? TokenType.EQUAL_EQUAL : TokenType.EQUAL);
      case '!' -> addToken(match('=') ? TokenType.NOT_EQUAL : TokenType.UNKNOWN);
      case '<' -> addToken(TokenType.LESS);
      case '>' -> addToken(TokenType.GREATER);
      case '-' -> addToken(match('>') ? TokenType.ARROW : TokenType.UNKNOWN);
      case '"' -> addStringLiteral();
      default -> {
